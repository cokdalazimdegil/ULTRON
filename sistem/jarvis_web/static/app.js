  ...
  ws.onmessage = (ev) => {
    ...
    switch (obj.type) {
      ...
      case "speaker_identified":
        setIdentifiedSpeaker(obj.user, obj);
        break;
...
// ── Otomatik Çok Modlu Biyometrik Doğrulama Göstergesi ───────────────────
// Oturum kilidi aktifken sunucudan gelen otomatik kimlik değişikliğini yok say
let _sessionIdentityLocked = false;

function setIdentifiedSpeaker(userName, meta = {}) {
  if (!userName) return;

  // Oturum kilidi aktifse ve bu otomatik bir tespit ise (MANUAL değilse) yok say
  const isManual = meta.state === "MANUAL";
  if (_sessionIdentityLocked && !isManual) {
    return;  // Manuel seçim korunur — otomatik değişiklik engellenir
  }

  // Oturum kilit durumunu güncelle
  if (meta.session_locked !== undefined) {
    _sessionIdentityLocked = meta.session_locked;
    // Toggle butonunu güncelle
    const lockIcon = $("identity-lock-icon");
    if (lockIcon) lockIcon.textContent = _sessionIdentityLocked ? "🔒" : "🔓";
  }

  const isUnknown = userName.toLowerCase() === "bilinmeyen" || userName.toLowerCase() === "unknown";
  S.currentUser = isUnknown ? "Bilinmeyen" : userName;
  if (currentUserDisplay) currentUserDisplay.textContent = S.currentUser;
  
  if (isUnknown) {
    stateManager.pushTransientState("UNKNOWN_SPEAKER", 2500, "YETKİSİZ KİŞİ // UNKNOWN");
  } else if (userName.toLowerCase().includes("rabia")) {
    stateManager.pushTransientState("VERIFIED_RABIA", 2200, "DOĞRULANDI: RABİA");
  } else {
    stateManager.pushTransientState("VERIFIED_NURI", 2200, `DOĞRULANDI: ${userName.toUpperCase()}`);
  }

  if (!speakerChip) return;

  if (isUnknown) {
    speakerChip.textContent = `🛡️ KİMLİK: TANIMLANAMADI`;
  } else {
    const vScore = meta.voice_score ? `🎙️%${Math.round(meta.voice_score * 100)}` : "";
    const sScore = meta.style_score ? `💬%${Math.round(meta.style_score * 100)}` : "";
    const cScore = meta.visual_score ? `📷%${Math.round(meta.visual_score * 100)}` : "";
    const factors = [vScore, sScore, cScore].filter(Boolean).join(" ");
    const lockLabel = _sessionIdentityLocked ? " 🔒" : "";
    speakerChip.textContent = `🛡️ ${userName.toUpperCase()} ${factors ? `[${factors}]` : ""}${lockLabel}`;
  }

  speakerChip.classList.remove("hidden");
  speakerChip.classList.toggle("unknown", isUnknown);
  speakerChip.classList.toggle("rabia", userName.toLowerCase().includes("rabia"));
}

// Kullanıcıyı manuel olarak seç (UI toggle)
function selectUser(userName) {
  if (!S.ws || S.ws.readyState !== WebSocket.OPEN) return;
  _sessionIdentityLocked = true;
  const lockIcon = $("identity-lock-icon");
  if (lockIcon) lockIcon.textContent = "🔒";
  S.ws.send(JSON.stringify({ type: "identify", user: userName }));
  setIdentifiedSpeaker(userName, { state: "MANUAL", session_locked: true });
}

// Kimliği sıfırla ve yeniden tespit başlat
function requestReIdentify() {
  if (!S.ws || S.ws.readyState !== WebSocket.OPEN) return;
  _sessionIdentityLocked = false;
  const lockIcon = $("identity-lock-icon");
  if (lockIcon) lockIcon.textContent = "🔓";
  S.ws.send(JSON.stringify({ type: "re_identify" }));
  if (speakerChip) speakerChip.textContent = "🔄 KİMLİK YENİLENİYOR...";
}
...
// ═══════════════════════════════════════════════════════════════════════════
// ── Biyometrik Ses Stüdyosu (Voice Profile Studio Controller) ─────────────
// ═══════════════════════════════════════════════════════════════════════════

const btnVoiceModal = $("btn-voice-modal");
const btnCloseVoice = $("btn-close-voice");
const voiceModal = $("voice-modal");
const voiceProfilesList = $("voice-profiles-list");
const btnStartEnrollRecord = $("btn-start-enroll-record");
const btnTestVoiceRecognition = $("btn-test-voice-recognition");
const btnWipeAllProfiles = $("btn-wipe-all-profiles");
const voiceNameInput = $("voice-name-input");
const voiceRoleInput = $("voice-role-input");
const voiceMeterWrap = $("voice-meter-wrap");
const voiceMeterBar = $("voice-meter-bar");
const voiceCountdownText = $("voice-countdown-text");
const voiceFeedbackMsg = $("voice-feedback-msg");
const voiceVisualizerCanvas = $("voice-visualizer-canvas");
const voiceVuLevel = $("voice-vu-level");

function drawVoiceVisualizer(pcmChunk) {
  if (!voiceModal || voiceModal.classList.contains("hidden")) return;
  if (!voiceVisualizerCanvas || !pcmChunk || pcmChunk.length === 0) return;

  const ctx = voiceVisualizerCanvas.getContext("2d");
  if (!ctx) return;

  const w = voiceVisualizerCanvas.width;
  const h = voiceVisualizerCanvas.height;
  ...
  const step = Math.max(1, Math.floor(pcmChunk.length / w));
  ctx.lineWidth = 2;
  ctx.strokeStyle = S.studioRecordingActive ? "#ff4422" : "#00ddff";
  ctx.shadowBlur = 6;
  ctx.shadowColor = S.studioRecordingActive ? "rgba(255, 68, 34, 0.8)" : "rgba(0, 221, 255, 0.8)";
  ctx.beginPath();

  let sumSquares = 0;
  for (let i = 0; i < w; i++) {
    const idx = i * step;
    if (idx >= pcmChunk.length) break;
    const sample = pcmChunk[idx] / 32768.0;
  ...
  const rms = Math.sqrt(sumSquares / Math.max(1, w));
  const pct = Math.min(100, Math.round(rms * 280));

  if (voiceVuLevel) {
    if (pct > 25) {
      voiceVuLevel.innerHTML = `<span style="color:var(--green)">● SİNYAL: %${pct} (SES İYİ)</span>`;
    } else if (pct > 8) {
      voiceVuLevel.innerHTML = `<span style="color:var(--amber)">● SİNYAL: %${pct} (NORMAL)</span>`;
    } else {
      voiceVuLevel.innerHTML = `<span style="color:var(--dim)">● SİNYAL: %${pct} (DÜŞÜK / FISILTI)</span>`;
    }
  }
}

if (btnVoiceModal) {
  btnVoiceModal.addEventListener("click", () => {
    if (voiceModal) voiceModal.classList.remove("hidden");
    loadVoiceStudioProfiles();
    if (!S.micOn) {
      ensurePlayCtx();
      startMic();
    }
...
async function loadVoiceStudioProfiles() {
  if (!voiceProfilesList) return;
  voiceProfilesList.innerHTML = `<div class="voice-loading">Profiller yükleniyor...</div>`;
  try {
    const res = await (await fetch("/api/voice/profiles")).json();
    const profiles = res.profiles || [];
    if (profiles.length === 0) {
      voiceProfilesList.innerHTML = `
        <div style="grid-column: 1/-1; color: var(--dim); font-size: 12px; padding: 12px; background: rgba(0,0,0,0.4); border-radius: 6px; border: 1px dashed var(--glass-border);">
          ℹ️ Henüz kayıtlı bir ses profili yok. İsteyen herkes aşağıdaki forma adını ve rolünü yazıp '4 SANİYELİK KAYDI BAŞLAT' butonuyla kendi biyometrik profilini oluşturabilir.
        </div>`;
      return;
    }

    voiceProfilesList.innerHTML = profiles.map(p => {
      const isRabia = p.name.toLowerCase().includes("rabia");
      const cardClass = isRabia ? "voice-profile-card rabia" : "voice-profile-card";
      const f0Text = p.median_f0 > 0 ? `${p.median_f0} Hz` : "Analiz Ediliyor";
      const genderClass = p.gender_hint === "female" ? "female" : (p.gender_hint === "male" ? "male" : "");
      const genderText = p.gender_hint === "female" ? "Kadın 👩" : (p.gender_hint === "male" ? "Erkek 👨" : "Bilinmiyor");
        ...
        <div class="${cardClass}">
          <div class="vp-header">
            <div>
              ...
              <div class="vp-role">${escapeHtml(p.role || "Kayıtlı Kullanıcı")}</div>
            </div>
            <span class="vp-pill">${p.sample_count} Örnek</span>
          </div>
          <div class="vp-meta-row">
            <span class="vp-pill pitch">🎵 Pitch: ${f0Text}</span>
            <span class="vp-pill ${genderClass}">${genderText}</span>
          </div>
          <div class="vp-actions">
            <button class="btn-vp-action" onclick="window.selectVoiceProfileForEnroll('${escapeHtml(p.name)}', '${escapeHtml(p.role || "")}')">🔴 Yeniden Kaydet</button>
            <button class="btn-vp-del" onclick="window.deleteVoiceProfile('${escapeHtml(p.name)}')">🗑️</button>
          </div>
        </div>
      `;
    }).join("");
  } catch (err) {
    voiceProfilesList.innerHTML = `<div style="color:red;">Profiller alınamadı: ${escapeHtml(err.message)}</div>`;
  }
}

window.selectVoiceProfileForEnroll = (name, role) => {
  if (voiceNameInput) voiceNameInput.value = name;
  if (voiceRoleInput) voiceRoleInput.value = role;
  if (voiceFeedbackMsg) {
    voiceFeedbackMsg.textContent = `'${name}' için kayda hazır. '4 SANİYELİK KAYDI BAŞLAT' butonuna basın.`;
    voiceFeedbackMsg.style.borderLeftColor = name.toLowerCase().includes("rabia") ? "#ff66aa" : "var(--ultron)";
  }
};

window.deleteVoiceProfile = async (name) => {
  if (!confirm(`'${name}' ses profilini silmek istediğinize emin misiniz?`)) return;
  try {
    const res = await (await fetch("/api/voice/delete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name })
    })).json();
    if (voiceFeedbackMsg) voiceFeedbackMsg.textContent = res.message;
    loadVoiceStudioProfiles();
  } catch (err) {
...
if (btnWipeAllProfiles) {
  btnWipeAllProfiles.addEventListener("click", async () => {
    if (!confirm("DİKKAT: Kayıtlı TÜM ses profilleri kalıcı olarak silinecek. Emin misiniz?")) return;
    try {
      const res = await (await fetch("/api/voice/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
// ── 4 Saniyelik Biyometrik Ses Yakalama ───────────────────────────────────
async function capturePCMStream(seconds = 4.0, onProgress = null) {
  if (!S.micOn) {
    await startMic();
  ...
  return new Promise((resolve) => {
    const interval = setInterval(() => {
      ...
      const remainingSec = Math.max(0, (seconds - elapsed / 1000).toFixed(1));

      if (onProgress) onProgress(progress, remainingSec);

      if (elapsed >= totalMs) {
        clearInterval(interval);
        S.studioRecordingActive = false;

        const totalLen = S.studioChunks.reduce((acc, c) => acc + c.length, 0);
        const combined = new Int16Array(totalLen);
        let offset = 0;
        for (const chunk of S.studioChunks) {
...
if (btnStartEnrollRecord) {
  btnStartEnrollRecord.addEventListener("click", async () => {
    const name = voiceNameInput ? voiceNameInput.value.trim() : "";
    const role = voiceRoleInput ? voiceRoleInput.value.trim() : "";
    if (!name) {
      alert("Lütfen kişi adını yazın.");
      return;
    }

    if (btnStartEnrollRecord) btnStartEnrollRecord.disabled = true;
    if (voiceMeterWrap) voiceMeterWrap.classList.remove("hidden");
    if (voiceFeedbackMsg) {
      voiceFeedbackMsg.textContent = `🎙️ DİNLENİYOR: Lütfen mikrofona 4 saniye net konuşun...`;
      voiceFeedbackMsg.style.borderLeftColor = "var(--ultron)";
    }

    try {
      const b64 = await capturePCMStream(4.0, (progress, rem) => {
        if (voiceMeterBar) voiceMeterBar.style.width = `${progress * 100}%`;
        if (voiceCountdownText) voiceCountdownText.textContent = `KAYDEDİLİYOR: ${rem}s`;
      });

      if (voiceCountdownText) voiceCountdownText.textContent = "İŞLENİYOR & HİBRİT ANALİZ...";

      const res = await (await fetch("/api/voice/enroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, role, audio_b64: b64 })
      })).json();

      if (voiceFeedbackMsg) {
        voiceFeedbackMsg.textContent = res.message || "Kayıt tamamlandı!";
        voiceFeedbackMsg.style.borderLeftColor = "var(--green)";
      }
      loadVoiceStudioProfiles();
    } catch (err) {
      if (voiceFeedbackMsg) {
        voiceFeedbackMsg.textContent = `Hata oluştu: ${escapeHtml(err.message)}`;
        voiceFeedbackMsg.style.borderLeftColor = "red";
      }
    } finally {
      if (btnStartEnrollRecord) btnStartEnrollRecord.disabled = false;
      if (voiceMeterWrap) voiceMeterWrap.classList.add("hidden");
    }
  });
}
...
if (btnTestVoiceRecognition) {
  btnTestVoiceRecognition.addEventListener("click", async () => {
    if (btnTestVoiceRecognition) btnTestVoiceRecognition.disabled = true;
    if (voiceMeterWrap) voiceMeterWrap.classList.remove("hidden");
    if (voiceFeedbackMsg) {
      voiceFeedbackMsg.textContent = `🧪 TEST: Mikrofona 3 saniye konuşun...`;
      voiceFeedbackMsg.style.borderLeftColor = "#00ddff";
    }

    try {
      const b64 = await capturePCMStream(3.0, (progress, rem) => {
        if (voiceMeterBar) voiceMeterBar.style.width = `${progress * 100}%`;
        if (voiceCountdownText) voiceCountdownText.textContent = `TEST EDİLİYOR: ${rem}s`;
      });

      if (voiceCountdownText) voiceCountdownText.textContent = "HİBRİT BİYOMETRİK DEĞERLENDİRME...";

      const res = await (await fetch("/api/voice/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.status === "ok") {
        const meta = res.meta || {};
        const pInfo = meta.f0_telemetry || meta.pitch_info || {};
        const qRep = meta.quality_report || {};
        const f0 = pInfo.median_f0 ? `${pInfo.median_f0} Hz (${pInfo.gender_hint === 'male' ? 'Erkek 👨' : (pInfo.gender_hint === 'female' ? 'Kadın 👩' : 'Nötr')}) [Telemetri]` : "Telemetri Yok";
        const matchScore = meta.best_similarity !== undefined ? meta.best_similarity.toFixed(2) : "0.00";
        const top2Text = meta.top2_speaker ? `${escapeHtml(meta.top2_speaker)}: ${(meta.top2_score || 0).toFixed(2)}` : "Yok (Tek Profil)";
        const marginText = meta.margin !== undefined ? `+${meta.margin.toFixed(2)}` : "0.00";
        const qStatus = qRep.status || "OK";
        const qSnr = qRep.snr_db !== undefined ? `${qRep.snr_db.toFixed(1)} dB` : "-";

        const breakdown = meta.hybrid_breakdown || {};
        let barsHtml = "";
        for (const [sName, sData] of Object.entries(breakdown)) {
          const isNuri = sName.toLowerCase().includes("nuri");
          const barClass = isNuri ? "nuri" : "rabia";
          const scoreVal = sData.match_score !== undefined ? sData.match_score : (sData.final_score || 0);
          const barPct = Math.max(0, Math.min(100, Math.round(scoreVal * 100)));
          const protoCount = sData.prototype_count || 1;
          barsHtml += `
            <div class="diag-bar-wrap">
              <div class="diag-bar-label">
                <span><strong>${escapeHtml(sName)}</strong> (${protoCount} Prototip)</span>
                <span>Skor: <strong>${scoreVal.toFixed(2)}</strong> (Top-K: ${(sData.top_k_mean || scoreVal).toFixed(2)}, Centroid: ${(sData.centroid_sim || scoreVal).toFixed(2)})</span>
              </div>
              <div class="diag-bar-bg">
                <div class="diag-bar-fill ${barClass}" style="width: ${barPct}%"></div>
              </div>
            </div>
          `;
        }

        const isAuthorized = res.identified_speaker && res.identified_speaker.toLowerCase() !== "bilinmeyen" && res.identified_speaker.toLowerCase() !== "unknown";
        const authBadge = isAuthorized
          ? `<span style="color:var(--green); font-weight:bold;">✅ DOĞRULANDI (VERIFIED)</span>`
          : `<span style="color:#ff4422; font-weight:bold;">🔒 TANINMAYAN SES (UNKNOWN)</span>`;

        const authDesc = isAuthorized
          ? `Biyometrik doğrulama başarılı. ULTRON tüm komutlarınızı yerine getirecektir.`
          : `Ses profili eşleşmedi veya güvenlik marjı yetersiz. ULTRON yetkisiz seslerle komut çalıştırmayı reddedecektir.`;

        voiceFeedbackMsg.innerHTML = `
          <div style="font-size:13px; font-weight:bold; margin-bottom:6px; display:flex; justify-content:space-between; align-items:center;">
            <span>🎯 TESPİT: <span style="color:var(--amber); font-size:14px;">${escapeHtml(res.identified_speaker)}</span></span>
            ${authBadge}
          </div>
          <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap:6px; background:rgba(0,0,0,0.3); padding:6px 8px; border-radius:4px; font-size:11px; margin-bottom:8px;">
            <div>🏆 <strong>Match Skoru:</strong> <span style="color:var(--cyan); font-weight:bold;">${matchScore}</span></div>
            <div>🥈 <strong>Top-2:</strong> <span>${top2Text}</span></div>
            <div>⚖️ <strong>Margin (Δ):</strong> <span style="color:var(--green); font-weight:bold;">${marginText}</span></div>
            <div>🎙️ <strong>Kalite (SNR):</strong> <span>${qStatus} (${qSnr})</span></div>
            <div>🔄 <strong>Durum:</strong> <span>${escapeHtml(res.state)}</span></div>
            <div>🎵 <strong>F0 Pitch:</strong> <span>${f0}</span></div>
          </div>
          <div style="font-size:11px; margin-bottom:8px; color:${isAuthorized ? 'var(--green)' : '#ff8888'};">
            ${authDesc}
          </div>
          ${barsHtml}
        `;
        voiceFeedbackMsg.style.borderLeftColor = isAuthorized ? "var(--green)" : "red";
      } else {
        voiceFeedbackMsg.textContent = res.message || "Test başarısız oldu.";
        voiceFeedbackMsg.style.borderLeftColor = "red";
      }
    } catch (err) {
      if (voiceFeedbackMsg) {
        voiceFeedbackMsg.textContent = `Test hatası: ${escapeHtml(err.message)}`;
        voiceFeedbackMsg.style.borderLeftColor = "red";
      }
    } finally {
      if (btnTestVoiceRecognition) btnTestVoiceRecognition.disabled = false;
      if (voiceMeterWrap) voiceMeterWrap.classList.add("hidden");
    }
  });
}
