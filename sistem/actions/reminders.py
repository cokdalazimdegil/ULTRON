...
def add_reminder(
    ...
    # Proaktif motoru otomatik olarak tetikle
    if normalized_due:
        try:
            from actions.proactive_engine import set_proactive_timer
            set_proactive_timer(title=title.strip(), due_iso=normalized_due)
        except Exception:
            pass
...
def delete_reminder(title: str, list_name: str = "") -> str:
    """
    Belirtilen başlıkla eşleşen hatırlatıcıyı siler.
    Windows'ta proactive_engine'den zamanlayıcı iptal eder;
    macOS'ta AppleScript ile Reminders.app üzerinden siler.
    """
    if not title or not title.strip():
        return "Hata: Silinecek hatırlatıcı başlığı belirtilmedi."

    title = title.strip()
    deleted_any = False
    results = []

    # 1. Proaktif zamanlayıcıyı iptal et
    try:
        from actions.proactive_engine import cancel_timer
        cancel_result = cancel_timer(title)
        if "iptal" in cancel_result.lower() or "silindi" in cancel_result.lower():
            results.append(f"✅ Zamanlayıcı iptal edildi: {title}")
            deleted_any = True
        else:
            results.append(f"ℹ️ Zamanlayıcı: {cancel_result}")
    except Exception as e:
        results.append(f"ℹ️ Zamanlayıcı iptal edilemedi: {e}")

    # 2. macOS Reminders.app AppleScript
    try:
        import platform
...
tell application "Reminders"
    try
        set rem to first reminder of {list_clause} whose name contains "{title}"
        delete rem
        return "silindi"
    on error errMsg
        return "hata: " & errMsg
...
"""
            out = subprocess.check_output(["osascript", "-e", script], text=True, timeout=8).strip()
            if out.startswith("silindi"):
                results.append(f"✅ Reminders.app'ten silindi: {title}")
                deleted_any = True
            else:
                results.append(f"ℹ️ Reminders.app: {out}")
    except Exception as e:
        results.append(f"ℹ️ macOS Reminders silme: {e}")

    if not results:
        return f"'{title}' başlıklı hatırlatıcı bulunamadı."

    return "\n".join(results) if deleted_any else f"'{title}' için silme tamamlanamadı.\n" + "\n".join(results)
