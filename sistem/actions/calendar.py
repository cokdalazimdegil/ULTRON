...
macOS   → Apple Calendar (EventKit, Swift yardimci uygulama)
Windows → actions/win_organizer.py (Outlook COM veya JARVIS yerel takvimi)
...
def _window_bounds(window: dict) -> tuple[dt.datetime, dt.datetime]:
    """Sorgu penceresini somut baslangic/bitis tarihlerine cevirir (Windows yolu)."""
    now = dt.datetime.now()
    today = now.replace(hour=0, minute=0, second=0, microsecond=0)
    mode = window["helper_mode"]

    if mode == "range" and window.get("payload"):
        payload = window["payload"]
        try:
            return (
                dt.datetime.fromisoformat(payload["start_iso"]),
                dt.datetime.fromisoformat(payload["end_iso"]),
            )
        except Exception:
...
# ═══════════════════════════════════════════════════════════════════════════
# macOS — Swift EventKit yardimcisi
# ═══════════════════════════════════════════════════════════════════════════
...
def _run_helper(mode: str, payload: dict | None = None, timeout: int = 20) -> tuple[bool, str]:
    """macOS EventKit yardimcisini calistirir. (reminders.py de kullanir.)"""
    if IS_WIN:
        return False, "Bu yardimci yalnizca macOS'ta kullanilir."
...
def _calendar_permission_message() -> str:
    if IS_WIN:
        return (
            "Takvime erisilemedi. JARVIS kendi yerel takvimini kullanir; "
            "Outlook takvimini kullanmak istersen Outlook'u acip bir hesap "
...
# ═══════════════════════════════════════════════════════════════════════════
# Bicimlendirme — platformdan bagimsiz
# ═══════════════════════════════════════════════════════════════════════════
...
def get_calendar_events(query: str = "today", limit: int = 6) -> str:
    ...
    if IS_WIN:
        ...
        ok, detail, events = win_organizer.fetch_events(start, end)
        if not ok:
...
def add_calendar_event(
    ...
    if IS_WIN:
        ...
        ok, detail, event = win_organizer.create_event(payload)
        if not ok:
...
def delete_calendar_event(
    ...
    if IS_WIN:
        ...
        ok, detail, event, matches = win_organizer.delete_event(payload)
        if not ok:
            now = dt.datetime.now()
            if matches:
                preview = " | ".join(_format_event_line(item, now) for item in matches[:3])
                detail = f"{detail} Eslesen etkinlikler: {preview}"
